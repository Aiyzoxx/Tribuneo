...
const BODACC_EXPORT_URL = 'https://bodacc-datadila.opendatasoft.com/api/explore/v2.1/catalog/datasets/annonces-commerciales/exports/csv';
const WHERE_CLAUSE = `(familleavis_lib:"Procédures collectives" OR familleavis_lib:"Ventes et cessions") AND numerodepartement:"85"`;
const BATCH_SIZE = 2000; // Reduced to 2000 to prevent API Adresse timeout
...
async function geocodeBatch(batch) {
  // Create a temporary CSV for geocoding
  const tempCsvPath = path.join(__dirname, 'temp_batch.csv');
...
async function runBulkImport() {
  console.log('--- DÉMARRAGE IMPORTATION MASSIVE ---');
  
  const MAX_RECORDS = process.argv[2] ? parseInt(process.argv[2]) : null; // Optionnel : limiter le nombre total
  if (MAX_RECORDS) console.log(`Limite configurée à ${MAX_RECORDS} dossiers max.`);
  ...
  try {
    console.log('Lecture du fichier CSV local (bodacc_vendee.csv)...');
    
    if (!fs.existsSync(path.join(__dirname, '../../bodacc_vendee.csv'))) {
      console.error("Le fichier bodacc_vendee.csv n'a pas été trouvé à la racine du dossier backend !");
      process.exit(1);
    }

    const stream = fs.createReadStream(path.join(__dirname, '../../bodacc_vendee.csv'))
      .pipe(csv({ separator: ';' }));
    ...
    for await (const row of stream) {
      ...
      // Strict filter for Vendée only
      if (row.numerodepartement !== '85') {
        continue;
      }
      
      // Filter only liquidations and cessions
      const famille = row.familleavis_lib || '';
      if (!famille.includes('Procédures collectives') && !famille.includes('Ventes et cessions')) {
        continue;
      }
      ...
      if (batch.length >= BATCH_SIZE) {
        console.log(`Géocodage du lot de ${batch.length} dossiers...`);
        ...
        console.log(`Progression: ${totalProcessed} traités, ${totalInserted} insérés.`);
    ...
    if (batch.length > 0) {
      ...
      const inserted = await insertBatch(batch, geocoded);
      totalInserted += inserted;
      console.log(`Progression finale: ${totalProcessed} traités, ${totalInserted} insérés.`);
    }
    
    console.log('--- IMPORTATION MASSIVE TERMINÉE ---');
    process.exit(0);
